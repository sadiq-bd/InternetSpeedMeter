...
install() {
  ...
  mkdir -p "${INSTALL_DIR}"
  rm -rf "${INSTALL_DIR}/InternetSpeedMeter@sadiq.us.to"
  cp -rf "${SRC_DIR}/src" "${INSTALL_DIR}/InternetSpeedMeter@sadiq.us.to" &>> "$LOG_FILE"
  is_failed "Done" "Skipping: Can not install to ${INSTALL_DIR}. See log for more info."
}

# build for release
build() {
  print "Creating InternetSpeedMeter@sadiq.us.to.zip"
  mkdir -p "${SRC_DIR}/out"
  zip -6rXj "$SRC_DIR/out/InternetSpeedMeter@sadiq.us.to.zip" "src" &>> "$LOG_FILE"
